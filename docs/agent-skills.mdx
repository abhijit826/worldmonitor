...
description: "Human-readable catalog of the 26 World Monitor agent skills advertised through the /.well-known/agent-skills/index.json discovery endpoint."
---

The catalog contains 26 World Monitor agent skills for agents that need a focused, ready-to-run recipe instead of a full OpenAPI crawl. Each skill is a public `SKILL.md` file with when-to-use guidance, auth shape, REST examples, response notes, and content-safety handling.
...
| `Iran.skill` | Geopolitical analysis OS with 2,500-year historical patterns, analyst models, and decision-maker predictions. | [SKILL.md](https://worldmonitor.app/.well-known/agent-skills/Iran.skill/SKILL.md) |
